...
async function getUsuarios() {
  try {
    const response = await axios.get(
      "http://localhost:8080/api/coordenador/usuarios"
    );
...
const handleDeleteUser = async (userId: string) => {
  try {
    const response = await axios.delete(
      `http://localhost:8080/api/coordenador/usuarios/${userId}`
    );
    if (response.status >= 200 && response.status < 300) {
...
const onSubmit = handleSubmit(async (values) => {
  const { email, nome, senha, cpf, dataNascimento, tipoUsuario, matricula } =
    values;
  ...
  try {
    const response = await axios.post(
      "http://localhost:8080/api/coordenador/register-user",
      usuario
    );
            ...
            <DialogContent>
              <form class="space-y-6" @submit="onSubmit">
                        ...
                        <PopoverTrigger as-child>
                          ...
                          >
                            <span>{{
                              dataNascimento
                                ? df.format(
                                    dataNascimento.toDate(getLocalTimeZone())
                                  )
                ...
                >
                  <FormItem>
                    <FormLabel
                      >Qual o tipo de perfil que deseja criar?
                      <span class="text-red-500 font-bold">*</span></FormLabel
                    >
                    <FormControl>
                      <RadioGroup class="flex gap-3" v-bind="tipoUsuario">
                        ...
                        <FormItem class="flex items-center gap-2">
                          ...
                          <FormLabel class="font-normal"> Professor </FormLabel>
                        </FormItem>
                      </RadioGroup>
                    </FormControl>
                  </FormItem>
                ...
                <FormField
                  v-if="values.tipoUsuario === 'ALUNO'"
                  v-slot="{ componentField: matricula }"
                  name="matricula"
                  :validate-on-blur="!isFieldDirty"
                >
                  <FormItem>
